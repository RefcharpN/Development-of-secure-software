...
//динамичный многопоточный брутфорс
...
//динамичный многопоточный брутфорс md5
void MainWindow::on_pushButton_16_clicked()
{
    this->ui->label_26->clear();

    int threadCount = this->ui->comboBox_2->currentText().toInt();
    int alphPart = 26/threadCount;
    QVector<BfThreadMD5*> tasks;
    ...
    for(int i = 0; i < 26; i = i + alphPart)
    {
        ...
        //BfThread thread = BfThread("test", char('a' + start), char('a' + end));
        tasks.append(new BfThreadMD5("test", char('a' + start), char('a' + end)));
    }
    ...
    for(int i = 0; i < tasks.length(); i++)
    {
        ...
        QObject::connect(tasks.at(i), &BfThreadMD5::send, this, &MainWindow::getWordMD5);
    }

    this->threadTimer.restart();
}

//однопоточный брутфорс md5
void MainWindow::on_pushButton_17_clicked()
{
    this->ui->label_32->clear();
    BfThreadMD5* thread = new BfThreadMD5("test", 'a', 'z');
    QObject::connect(thread, &BfThreadMD5::send, this, &MainWindow::getWordMonoMD5);
    thread->start();
    this->threadTimer.restart();
}

void MainWindow::getWordMD5(QString word)
{
    this->ui->label_26->setText(this->ui->label_26->text()+ QCryptographicHash::hash(word.toUtf8(), QCryptographicHash::Md5).toHex() + " - " + word + "\n");

    this->threadCount++;
    if(this->threadCount == 3)
    {
        this->ui->label_26->setText(this->ui->label_26->text() + QString("время выполения - %1 секунд\n").arg(this->threadTimer.elapsed()/1000));
        this->threadCount = 0;
    }
}

void MainWindow::getWordMonoMD5(QString word)
{
    this->ui->label_32->setText(this->ui->label_32->text()+ QCryptographicHash::hash(word.toUtf8(), QCryptographicHash::Md5).toHex() + " - " + word + "\n");

    this->threadCount++;
    if(this->threadCount == 3)
    {
        this->ui->label_32->setText(this->ui->label_32->text() + QString("время выполения - %1 секунд\n").arg(this->threadTimer.elapsed()/1000));
        this->threadCount = 0;
    }
}
